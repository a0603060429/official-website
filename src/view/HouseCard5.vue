            ...
            <div v-for="(item,index) in dataList" :key="index" style="width:30%;height:320px;margin:20px 0">
                <div class="thumbnail" style="display:flex;height:100%;border-radius: 28px;box-shadow: 2px 2px 8px 0 rgba(0, 0, 0, 0.2);">
                    ...
                    <div class="caption" style="width:50%;position: relative;">
                        <div>
                            <H4 class="col-sm-12"><b>{{item.address}}</b></H4>
                            <p class="col-sm-12" v-for="(k) in (item.transportation || '').split(',', 2)">{{k}}</p>
                            <span class="card_tag">土地面積&nbsp;&nbsp;&nbsp;{{item.area}}</span>
                            <span class="card_tag">用途地域&nbsp;&nbsp;&nbsp;{{item.use}}</span>
                            <span class="card_tag">建ぺい率/容積率&nbsp;&nbsp;&nbsp;{{item.rate}}</span>
                            <span class="card_tag">前面道路&nbsp;&nbsp;&nbsp;{{item.theWay}}</span>
                        </div>
                        <div class="price_div">
                            <span class="price">
...
export default {
  ...
  methods: {
    async loadData(){
        let res = await api.getRealEstateDatas({realEstateType:5});
